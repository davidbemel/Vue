<template >
  <div>
    <div class='row mt-2 text-left' v-for="(task, index) in tasks" v-bind:key='index'>
        ...
        <span v-bind:id='"agregar"'
            v-bind:class='["mr-1","btn","btn-primary", agregar]'
            style="font-size: 8px"
            v-on:click='manejoClick($event)'></span>
        <span v-bind:id='"eliminar-"+index'
            v-bind:class='["btn","btn-primary", eliminar]'
            style="font-size: 8px"
            v-on:click='manejoClick($event)'></span>
        </div>
    </div>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        tasks: [{nombre:'Estudiar componentes en Vue',avance:27, esperado:45},
